...
" + Helpers + PreChecks + @"
    ...
    elseif class == 'PALADIN' and t3 >= t1 and t3 >= t2 then
        ...
        -- Священный щит (только при HP < 80%)
        local php = UnitHealth('player')/UnitHealthMax('player')
        if WB_S.SS~=false and php < 0.8 and not HasBuff('Священный щит') and IsReady('Священный щит') then CastSpellByName('Священный щит') return end
    end
end
WB_Run()
";
