...
## Известные проблемы
- Follow и Rotation связаны — follow работает только при запущенной ротации. Нужен рефакторинг: вынести follow-тик в отдельный цикл
- Facing во время каста: C# поворачивает к follow даже пока идёт каст → касты могут не проходить. Пробовали: таймер, isMoving, задержку — всё ломало что-то другое. Корень: C# не знает кастуем ли мы (не можем читать Lua переменные)
- CheckInteractDistance НЕ работает на WoWCircle
- SetFollowFromFocus: TargetUnit('focus') работает, но нужна пауза Thread.Sleep(200) для обновления в памяти

## Планы (следующие этапы)
- [ ] Отделить follow от rotation — независимые циклы
- [ ] Фикс facing во время каста
- [ ] Дубовая кожа по HP — автокаст дефов когда HP < 50%
- [ ] Хоткей вкл/выкл — F1 для toggle
- [x] Оверлей ✅
- [ ] Hot-reload ротаций — загрузка .lua файлов без перекомпиляции
