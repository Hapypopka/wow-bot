...
- [ ] **FIX: VT double-cast** — мультидот кастует VT 2 раза на одну цель (не ждёт GCD перед следующим тиком)
- [ ] **Секция "Таргет"** — переименовать "Автоповорот" → "Таргет" с двумя чекбоксами: "Автоповорот к таргету" + "Автовыбор таргета" (если таргет умер → автоматически выбирать следующего врага)
