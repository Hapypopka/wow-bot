...
- [x] **FIX: VT double-cast** ✅ — защита через WB_VT/WB_DP/WB_SWP глобальные Lua таймеры
- [x] **Секция "Таргет"** ✅ — автоповорот + автовыбор + слайдер дальности
- [ ] **Мультибоксинг (Hivemind)** — один мастер-бот управляет несколькими WoW клиентами: общий follow, атака таргета мастера, баффы группы. Панель со всеми персонажами
- [ ] Ротации для кастеров: Demo, Affli, Destro, Fire Mage, Arcane Mage, Elem Shaman
- [ ] Выбор спека в оверлее (dropdown)
- [ ] Дубовая кожа по HP
- [ ] Хоткей вкл/выкл
- [ ] Hot-reload ротаций
- [ ] Поддержка fullscreen (альтернативный метод поиска EndScene)
